...
This is a within subject design experiment with two conditions. In the first condition participants answer questions related to causal relationships in node-link graphs, while in the second one they are asked to answer questions on matrix-based representation. Each condition has four levels - size, layout, question type and question domain. The dependent variables were time taken to answer each question and correctness of the answer. The participants were also given a questionnaire which requires subjective answers about their graph preferences and how they enjoy solving logical problems in general. 

\section{Stimuli}

Figure \ref{experimentalDesign} illustrates the pattern which was used create the questions and the graphs. The domain questions are healthy and unhealthy gym behaviour, drinking issues, and student exams. The three different sizes were small (10 factors), medium (20 factors) and large (30 factors). There are three different layouts for each type of representation. The matrix-based layouts are alphabetical, in degree descending and out degree descending. The node-link layouts are hierarchical, orthogonal, and series parallel.  There was a direct relationship question and an indirect causal relationship question asked for each of the created graphs. The first six questions of the experiment are trial questions which do not count towards the final results. They are added to the 36 experimental questions generating a total of 42 questions.
